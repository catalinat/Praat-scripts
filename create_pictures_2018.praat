...
beginPause: "Where are your files"
    comment: "Enter parent directory where sound files are kept:"
 	...
 	sentence: "textDir" , "'baseDir$'/TG"
  	comment: "Enter directory to which created pictures should be saved:"
 	sentence: "picDir" , "'baseDir$'/NEW"
...
Create Strings as file list... mywavlist 'soundDir$'/*.wav
...
for ifile to numberOfFiles
	...
	# Reads Sound
	Read from file... 'soundDir$'/'base$'.wav
...
#######################		DIBUJA EL TEXTGRID		####################################
	if draw_TextGrid = 1
		if fileReadable (textDir$ + "/" + base$ +".TextGrid" )
			Read from file... 'textDir$'/'base$'.TextGrid
  ...
  #############################		GUARDA LA IMAGEN ##############################
	...
	if picture_Format = 1
		Save as 300-dpi PNG file: picDir$ + "/" + base$ + ".png"
	endif

	if picture_Format = 2
		Save as 600-dpi PNG file: picDir$ + "/" + base$ + ".png"
	endif
	
  	if picture_Format = 3
  		Save as PDF file: picDir$ +"/"+ base$ + ".pdf"
	endif
	
	if picture_Format = 4
		Write to Windows metafile... 'picDir$'/'base$'.wmf
	endif

	if picture_Format = 5
		Write to EPS file... 'picDir$'/'base$'.eps
	endif

	if picture_Format = 6
		Write to praat picture file... 'picDir$'/'base$'.praapic
	endif
...
appendInfoLine: "The picture files are in " + picDir$